...
delete = (idxp, name) => {
 ...
 hideEdit = async () => {
    ...
    // Encoder les chaînes en UTF-8 avant de les ajouter à formData
    formData.append('txt', encodeURIComponent(this.state.txt));
    formData.append('trad', encodeURIComponent(this.state.trad));
    formData.append('description', encodeURIComponent(this.state.description));
    
    formData.append('id_groupe', userInfo.id_groupe);
    formData.append('targetLangue', this.state.targetLangue);
...
const PADDING = 12;
const BOX_SIZE = SPINNER_SIZE + PADDING * 2;
