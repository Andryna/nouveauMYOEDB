...
class AdjList extends Component {
  ...
  renderSection = (title, data, sectionKey) => {
  ...
  render() {
    const { onlyVerbes1, onlyVerbes2, intersection } = this.state;
    const {uniqueFr, uniqueEn, commun} = this.props;

    return (
      <View style={styles.container}>
        {this.renderSection(uniqueFr, onlyVerbes1, 'onlyVerbes1')}
        {this.renderSection(uniqueEn, onlyVerbes2, 'onlyVerbes2')}
        {this.renderSection(commun, intersection, 'intersection')}
      </View>
    );
  }
}
