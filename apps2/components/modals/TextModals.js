...
import { View, Text, TouchableOpacity, ImageBackground, Modal, TextInput, ScrollView,Animated,Alert,Keyboard, Image, StyleSheet, ActivityIndicator } from 'react-native';
...
class TextModals extends Component {
  ...
  translatemytxt = async (text, targetLang) => {
    alert(targetLang);
  ...
  handleTranslate = async (itemValue) => {
    ...
    if (itemValue != '') {
      ...
      // const txt = await translate({ text: "Je suis très content", to: "es" });
      // alert(txt);
  ...
  readText = async (phrase, langue) => {
    ...
    speakTxt = async () => {
      ...
      console.log('speakTxt called with langue:', langue, 'and expres:', expres);
    
      if (!expres) return;
      this.readText(expres, infolangue); // phrase en anglais → TTS français
      ...
      speakTrad = async () => {
        ...
        try {
          await readText2(
            ...
            ({ isLoading, isPlaying }) => {
              // Tu peux mettre à jour ton state ou loader
              this.setState({ isLoadingTrad: isLoading, isPlayingTrad: isPlaying });
            },
            target_langue,       // fr / en
            'female',     // ou 'male'
            1.0           // vitesse
  ...
  transcript = async () => {
    ...
    try {
        ...
        .then(async (resp) => {
          this.handleTranslate(this.state.target_langue);
          const translated = await translateTexts(resp.data.result, this.state.target_langue);
          this.setState({trad:true,transcripts:false,expres:resp.data.result,targTEXT: translated, trad:true ,pathaudio:resp.data.audio,percentCompleted:'100',transcribed:true});
          // console.log('target langue', this.state.target_langue);
          console.log(this.state.resp);
          console.log('Transcription réussie:', resp.data);
          }) 
        } catch (err) {
...
writeTexte(){
  ...
  try {
    ...
    this.setState({
      ...
      // const path = audioFile.replace('file://', ''); // Supprimer le préfixe file://
      // alert(path);
      RNFS.stat(audioFile)
        .then(res => {
...
handleLanguageChange = () => {
  this.setState(
    ...
    () => {
      ...
      this.translatemytxt("Bonjour monsieur",this.state.langue);
    }
  );
};
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignSelf: 'center', width: wp('90%'), marginTop: hp('5%') }}>
                ...
                {!this.state.isLoading ? (
                  !this.state.isPlaying ? (
                    ...
                    <TouchableOpacity
                      onPress={() => {
                        // stop TTS / audio playback and update state
                        try { stopAudio(); } catch (e) { /* ignore */ }
                        if (this.sound && this.sound.stop) {
                          try { this.sound.stop(); } catch (e) { /* ignore */ }
                        }
                        this.setState({ isPlaying: false, inPlay: false });
                      }}
                      style={{ width: wp('20%'), alignItems: 'flex-end' }}
                    >
                  ...
                  <View style={{ width: wp('20%'), alignItems: 'flex-end' }}>
                    <ActivityIndicator size="small" color="#EA1E69" />
                  </View>
                )}
              </View>
                  ...
                  !this.state.isPlayingTrad ? (
                    ...
                    <TouchableOpacity
                      onPress={() => {
                        ...
                        this.setState({ isPlayingTrad: false, inPlay: false });
                      }}
                      style={{ width: wp('20%'), alignItems: 'flex-end' }}
                    >
                      <Icon name="volume-mute" size={25} color="white" style={{ alignSelf: 'flex-end', marginLeft: 5 }} />
